...
import { useGetDoctorByIDQuery, useUpdateDoctorMutation } from '../store/APIFeatures/DoctorApi'
...
const UpdateDoctor = ({ detail, setShowDialogConfig }) => {

    const { data: doctorDataById, isLoading, isSuccess } = useGetDoctorByIDQuery(detail)
    const [updateDoctor, {data:updatedDoctorData, isLoading:isUpdateDoctorLoading, isSuccess:isUpdateDoctorSuccess}] = useUpdateDoctorMutation(detail)

    const [doctorName, setDoctorName]= useState('')
    const [qualification, setQualification] = useState('')

    const handleCancelChange = () => {
        setShowDialogConfig(false);
    };
    const handleUpdateDoctor=()=>{
        updateDoctor({doctorName, qualification})
        setShowDialogConfig(false)
    ...
    useEffect(()=>{
        if(isSuccess){
            toast.success('Doctor Updated Sucessfully')
        }
    })
    return (
        <div className="fixed inset-0 z-30 flex  bg-black bg-opacity-10 font-manrope ">
            <div className="flex flex-col mx-[200px] my-[50px] w-full bg-white   ">
                ...
                <div className="flex justify-end px-10">
                    <button className='bg-button-default p-3 text-white rounded-border mt-4' onClick={handleUpdateDoctor}>Update</button>
